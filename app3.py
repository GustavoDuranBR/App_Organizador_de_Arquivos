...
class FileOrganizerApp:
    def __init__(self, root):
        ...
        # Variável para armazenar a referência da imagem do QR code
        self.qr_code_photo = None
    ...
    def resize_qr_code(self, qr_code_path):
        ...
        qr_code_image = qr_code_image.resize((new_width, new_height), Image.Resampling.LANCZOS)
    ...
    def organize_files(self):
        ...
        if not file_type:
            # Se o campo tipo de arquivo estiver vazio, pergunte ao usuário
            response = tk.messagebox.askyesno("Aviso", "O campo 'Tipo de Arquivo' está vazio. Deseja prosseguir sem especificar um tipo de arquivo?")
            if not response:
                return
            # Se o usuário escolher prosseguir sem especificar um tipo de arquivo,
            # então verifica se o campo parte do nome está preenchido
            if not name_part:
                tk.messagebox.showerror("Erro", "Preencha o campo 'Parte do Nome'.")
                return

            # Se o usuário escolher prosseguir sem especificar um tipo de arquivo,
            # pegue todos os arquivos da pasta de origem
            files_to_organize = [filename for filename in os.listdir(source_folder) if name_part in filename]
        else:
            # Se o campo tipo de arquivo não estiver vazio, verifica se o campo parte do nome está preenchido
